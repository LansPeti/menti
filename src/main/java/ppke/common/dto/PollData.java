 ...
 * (pl. opciók Multiple Choice esetén).
...
public class PollData implements Serializable {
    ...
    // Scale specifikus (NINCS HASZNÁLVA)
    ...
    /** @return A szempontok listájának másolata (Scale - nincs használva). Soha nem null. */
    public List<String> getAspects() { return aspects == null ? new ArrayList<>() : new ArrayList<>(aspects); }
    /** @return A skála minimum értéke (Scale - nincs használva). */
    public int getScaleMin() { return scaleMin; }
    /** @return A skála maximum értéke (Scale - nincs használva). */
    ...
    /** Beállítja a szempontokat (Scale - nincs használva). A kapott lista másolatát tárolja. Null lista esetén üres listát hoz létre. @param aspects A szempontok listája. @return Ez az objektum (láncoláshoz). */
    public PollData setAspects(List<String> aspects) { this.aspects = (aspects != null) ? new ArrayList<>(aspects) : new ArrayList<>(); return this; }
    /** Beállítja a skála minimum értékét (Scale - nincs használva). @param scaleMin A minimum érték. @return Ez az objektum (láncoláshoz). */
    public PollData setScaleMin(int scaleMin) { this.scaleMin = scaleMin; return this; }
    /** Beállítja a skála maximum értékét (Scale - nincs használva). @param scaleMax A maximum érték. @return Ez az objektum (láncoláshoz). */
